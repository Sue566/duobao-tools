...
const treeView = {
  ...
  appendObjectNode: function(parent, obj, keyName = null) {
    ...
    if (keyName !== null) {
      ...
      summary.innerHTML = `<span class="tree-key">${utils.escapeHtml(keyName)}</span><span class="tree-colon">: </span><span class="tree-object">{${keys.length} 个属性}</span>`;
  ...
  appendArrayNode: function(parent, arr, keyName = null) {
    ...
    if (keyName !== null) {
      ...
      const summary = document.createElement('summary');
      summary.innerHTML = `<span class="tree-key">${utils.escapeHtml(keyName)}</span><span class="tree-colon">: </span><span class="tree-array">[${arr.length} 个元素]</span>`;
  ...
  appendValueNode: function(parent, value, key) {
    ...
    if (value === null) {
      ...
      displayValue = `"${utils.escapeHtml(value)}"`;
    ...
    li.innerHTML = `<span class="tree-key">${utils.escapeHtml(String(key))}</span><span class="tree-colon">: </span><span class="${valueClass}">${displayValue}</span>`;
    parent.appendChild(li);
  },
