...
(function() {
  const formatter = {
    ...
    updateJsonStats: function(json, statsElements) {
      ...
      const nodes = window.jsonFormatter.utils.countNodes(json);
      const depth = window.jsonFormatter.utils.calculateDepth(json);
      
      jsonSize.textContent = window.jsonFormatter.utils.formatSize(size);
    ...
    processJson: function(json, options, elements) {
      try {
        ...
        jsonOutput.innerHTML = window.jsonFormatter.utils.syntaxHighlight(formattedJson);
  ...
  window.jsonFormatter.formatter = formatter;
})();