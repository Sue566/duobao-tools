  ...
  formatDiff: function(diff) {
    ...
    if (diff.added.length > 0) {
      ...
      for (const item of diff.added) {
        html += `<li class="diff-added"><strong>${item.path}</strong>: ${utils.formatDiffValue(item.value)}</li>`;
      }
      html += '</ul>';
    }
    ...
    if (diff.removed.length > 0) {
      ...
      for (const item of diff.removed) {
        html += `<li class="diff-removed"><strong>${item.path}</strong>: ${utils.formatDiffValue(item.value)}</li>`;
      }
      html += '</ul>';
    }
    ...
    if (diff.changed.length > 0) {
      ...
      for (const item of diff.changed) {
        html += `<li><strong>${item.path}</strong>: <span class="diff-removed">${utils.formatDiffValue(item.oldValue)}</span> → <span class="diff-added">${utils.formatDiffValue(item.newValue)}</span></li>`;
      }
      html += '</ul>';
    }
    ...
    if (diff.unchanged.length > 0) {
      ...
      for (const item of diff.unchanged) {
        html += `<li class="diff-unchanged"><strong>${item.path}</strong>: ${utils.formatDiffValue(item.value)}</li>`;
      }
      html += '</ul>';
    }
