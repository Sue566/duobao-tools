...
(function() {
  const compare = {
    ...
    compareHashes: function(container) {
      ...
      if (!hash1 || !hash2) {
        window.hashCalculator.utils.showToast('请输入两个哈希值进行比较', 'warning');
        return;
      }
    ...
    verifyFileHash: function(container) {
      ...
      if (!expectedHash) {
        window.hashCalculator.utils.showToast('请输入预期的哈希值', 'warning');
        return;
      }
  ...
  window.hashCalculator.compare = compare;
})();