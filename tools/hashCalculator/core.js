...
const core = {
  ...
  calculateTextHash: function(container) {
    ...
    // 获取选中的算法
    const algorithms = utils.getSelectedAlgorithms(container);
    if (algorithms.length === 0) {
      utils.showToast('请至少选择一种哈希算法', 'warning');
      return;
    }
    ...
    setTimeout(() => {
      try {
        ...
        // 显示结果
        utils.displayResults(container, results);
      } catch (e) {
    ...
    reader.onload = function(e) {
      ...
      setTimeout(() => {
        try {
          ...
          // 保存到历史记录
          utils.saveToHistory(file.name, results);
        } catch (e) {
          hashResults.innerHTML = `<div class="error">计算哈希值时出错: ${e.message}</div>`;
        }
      }, 100);
