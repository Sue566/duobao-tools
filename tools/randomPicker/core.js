...
(function() {
  ...
  window.randomPicker.core = {
    ...
    setSeed: function(seed) {
      ...
      // 简单的伪随机数生成器
      const seedNumber = window.randomPicker.utils.hashString(seed);
    ...
    pickRandomItems: function(container) {
      ...
      if (!input) {
        window.randomPicker.utils.showToast('请输入项目列表', 'warning');
        return;
      }
      
      const count = parseInt(pickCount.value);
      if (isNaN(count) || count < 1) {
        window.randomPicker.utils.showToast('请输入有效的选择数量', 'warning');
        return;
      }
      
      const items = window.randomPicker.utils.parseItems(input);
      if (items.length === 0) {
        window.randomPicker.utils.showToast('没有有效的项目', 'warning');
        return;
      }
      ...
      if (!allowRepeat && count > items.length) {
        window.randomPicker.utils.showToast(`选择数量不能超过项目数量 (${items.length})`, 'warning');
        return;
      }
      ...
      if (isShuffle) {
        ...
        if (shouldExcludePrevious && previousItems.length > 0) {
          ...
          if (availableItems.length === 0) {
            window.randomPicker.utils.showToast('排除上次结果后没有可选项目', 'warning');
            return;
          }
        }
      ...
      if (batchSize > 1) {
        ...
        // 显示批量结果
        window.randomPicker.ui.displayBatchResults(container, allResults, withWeights);
        
        // 添加到历史记录
        window.randomPicker.history.addBatchToHistory(allResults, withWeights);
      } else {
        // 显示单次结果
        window.randomPicker.ui.displayResults(container, selectedItems, withWeights);
        
        // 添加到历史记录
        window.randomPicker.history.addToHistory(selectedItems, withWeights);
      }
    },
    ...
    weightedRandomSelect: function(items, container) {
      ...
      if (distributionType === 'uniform') {
        ...
        try {
          ...
          const customProbs = window.randomPicker.utils.parseCustomDistribution(customDistribution);
    ...
    pickGroupItems: function(container) {
      ...
      if (!input) {
        window.randomPicker.utils.showToast('请输入分组定义', 'warning');
        return;
      }
      
      const groups = window.randomPicker.utils.parseGroups(input);
      if (groups.length === 0) {
        window.randomPicker.utils.showToast('没有有效的分组', 'warning');
        return;
      }
      ...
      if (mode === 'one-from-each') {
        ...
        if (isNaN(groupCountValue) || groupCountValue < 1) {
          window.randomPicker.utils.showToast('请输入有效的组数', 'warning');
          return;
        }
        
        if (isNaN(itemsPerGroupValue) || itemsPerGroupValue < 1) {
          window.randomPicker.utils.showToast('请输入有效的每组选择数量', 'warning');
          return;
        }
      ...
      // 显示分组结果
      window.randomPicker.ui.displayGroupResults(container, results);
      
      // 添加到历史记录
      window.randomPicker.history.addGroupToHistory(results);
    }
  };
})();