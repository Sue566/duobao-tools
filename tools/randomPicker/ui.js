  ...
  window.randomPicker.ui = {
    ...
    bindEvents: function() {
      ...
      this.listPickBtn.addEventListener('click', () => {
        window.randomPicker.core.pickFromList(
      ...
      this.listSaveBtn.addEventListener('click', () => {
        window.randomPicker.data.saveList(this.listInput.value);
      });
      
      this.listLoadBtn.addEventListener('click', () => {
        window.randomPicker.data.loadList(list => {
          if (list) {
      ...
      this.rangePickBtn.addEventListener('click', () => {
        window.randomPicker.core.pickFromRange(
      ...
      this.customPickBtn.addEventListener('click', () => {
        ...
        window.randomPicker.core.pickFromCustom(type, options);
      });
      ...
      this.groupPickBtn.addEventListener('click', () => {
        window.randomPicker.core.createGroups(
      ...
      this.historyClearBtn.addEventListener('click', () => {
        if (confirm('确定要清空所有历史记录吗？')) {
          window.randomPicker.history.clearHistory();
          this.loadHistory();
        }
      });
      
      this.historyExportBtn.addEventListener('click', () => {
        window.randomPicker.history.exportHistory();
      });
      
      // 结果操作事件
      this.copyResultBtn.addEventListener('click', () => {
        window.randomPicker.utils.copyToClipboard(this.resultDisplay.innerText);
      });
      
      this.saveResultBtn.addEventListener('click', () => {
        const result = this.resultDisplay.innerHTML;
        window.randomPicker.history.saveResult(result);
        this.loadHistory();
      });
