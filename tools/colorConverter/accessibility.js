...
(function() {
  const accessibility = {
    ...
    initAccessibility: function(container) {
      ...
      const checkContrast = () => {
        ...
        // 验证颜色格式
        if (!window.colorConverter.utils.isValidColor(foreground) || !window.colorConverter.utils.isValidColor(background)) {
  ...
  // 将可访问性检测添加到全局命名空间
  window.colorConverter.accessibility = accessibility;
})();